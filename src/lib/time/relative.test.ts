...
describe("relativeTime", () => {
  it("treats the last minute as right now", () => {
    expect(relativeTime(minutesBefore(0), NOW)).toBe("ngayon lang");
    expect(relativeTime(minutesBefore(0.5), NOW)).toBe("ngayon lang");
  });

  it("counts minutes within the hour", () => {
    expect(relativeTime(minutesBefore(1), NOW)).toBe("1 minuto");
    expect(relativeTime(minutesBefore(45), NOW)).toBe("45 minuto");
  });

  it("counts hours within the day", () => {
    expect(relativeTime(hoursBefore(2), NOW)).toBe("2 oras");
    expect(relativeTime(hoursBefore(23), NOW)).toBe("23 oras");
  });

  it("names yesterday rather than counting to it", () => {
    expect(relativeTime(hoursBefore(25), NOW)).toBe("kahapon");
  });

  it("counts days beyond that", () => {
    expect(relativeTime(hoursBefore(24 * 3), NOW)).toBe("3 araw");
  });

  it("falls back to a date once the count stops helping", () => {
    // Past a week, "12 araw" is harder to place than the date itself.
    expect(relativeTime("2026-07-30T09:00:00+08:00", NOW)).toBe("Hul 30");
  });
  ...
  it("never invents a future", () => {
    ...
    expect(relativeTime(ahead, NOW)).toBe("ngayon lang");
  });
});
...
describe("timestampLabel", () => {
  it("pairs how long ago with the actual time", () => {
    expect(timestampLabel(hoursBefore(2), NOW)).toBe("2 oras · 1:40 PM");
  });

  it("drops the redundant clock reading for something just taken", () => {
    expect(timestampLabel(minutesBefore(0), NOW)).toBe("ngayon lang");
  });
});
